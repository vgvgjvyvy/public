<template>
	<view>
		...
		<!-- 分类导航区域 -->
		<view class="nav-list">
			<view v-for="item in navList" :key="item.image_src" @click="handleNavClick(item)">
				<image :src="item.image_src" mode="" />
			</view>
		</view>
			...
			<view class="floor-item" v-for="(item, index) in floorList" :key="index">
				...
				<view class="floor-img-box">
					...
					<view class="floor-img-right">
						<navigator v-for="(img, index2) in item.product_list.slice(1)" :key="index2"
...
<script setup>
	...
	import {
		useCartBadge
	} from "@/composables/useCartBadge.js";
