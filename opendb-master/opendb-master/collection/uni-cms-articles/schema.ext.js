...
const { QuillDeltaToHtmlConverter, QuillDeltaToJSONConverter } = safeRequire(
...
function checkContentSecurityEnable(field) {
  ...
  return (
    ...
    config.contentSecurity.allowCheckType &&
    config.contentSecurity.allowCheckType.includes(field)
  );
}
...
function safeRequire(module) {
  try {
    ...
    if (e.code === "MODULE_NOT_FOUND") {
      throw new Error(
        `${module} 公共模块不存在，请在 uniCloud/database 目录右击"配置schema扩展公共模块"添加 ${module} 模块`,
      );
    }
  }
}
  ...
  trigger: {
    ...
    beforeCreate: async function ({ clientInfo, addDataList }) {
      ...
      for (const addData of addDataList) {
        ...
        if (allowCheckContent && addData.content) {
          parallel.push(
            checkContentSec(
              JSON.stringify(addData.content),
              clientInfo.requestId,
              "内容存在敏感字，请修改后提交",
            ),
          );
    ...
    afterRead: async function ({ userInfo, clientInfo, result, where, field }) {
      ...
      if ((!config.clientAppIds || !config.clientAppIds.length) && !isAdmin) {
        throw new Error(
          "请在 uni-cms 配置文件中配置 clientAppIds 字段后访问，详见：https://uniapp.dcloud.net.cn/uniCloud/uni-cms.html#uni-cms-config",
        );
      }
      ...
      if (
        isAdmin &&
        config.clientAppIds &&
        config.clientAppIds.includes(clientInfo.appId)
      ) {
        throw new Error(
          "uni-cms 配置文件中的 clientAppIds 字段请勿添加管理端的 appid，请移除后重试",
        );
      }

      // 如果是管理端请求，或当前appId不在clientAppIds中，则返回
      if (isAdmin || !config.clientAppIds.includes(clientInfo.appId)) return;
      ...
      // 获取文章内容中的图片
      article.content_images = article.content.ops.reduce(
        (imageBlocks, block) => {
          ...
          return imageBlocks.concat([parseCustom.source || block.insert.image]);
        },
        [],
      );
      ...
      // 获取唯一标识符
      const uniqueId =
        adConfig.watchAdUniqueType === "user"
          ? userInfo.uid
          : clientInfo.deviceId;
...
function getRenderableArticleContent(rawArticleContent, clientInfo) {
  ...
  if (!isUniAppX) {
    const quillDeltaConverter = new QuillDeltaToJSONConverter(
      rawArticleContent.ops,
    );
  ...
  for (let i = 0; i < rawArticleContent.ops.length; i++) {
    ...
    if (typeof op.insert === "object") {
      ...
      if (
        insertType &&
        insertType.length > 0 &&
        blockRenderList.includes(insertType[0])
      ) {
        deltaOps.push({
          type: insertType[0],
          ops: [op],
